...
class MarkerCommand: NSObject, XCSourceEditorCommand {
    ...
    func formattingMarks(in input: String) -> String? {
        ...
        if let completeMark = Range(match.range(withName: "completemark"), in: input),
            let _ = Mark(rawValue: String(input[completeMark])) {
            return nil
        }
        ...
        let replacementRange = mark.lowerBound..<upperBoundForReplacement
...
\\*                 allow some whitespace after comment
